import imgRecent from "./assets/unsplash_znCLdh5-Srk.png";
import imgRecent1 from "./assets/recent1.png";
import imgRecent2 from "./assets/recent2.png";
import imgBlog from "./assets/unsplash_PGnqT0rXWLs.png";

export const DATA_RECENTS = [
  {
    imgURL: imgRecent,
    date: "19 Mar",
    title:
      "Merevolusi penyimpanan energi: Terobosan dalam baterai berbasis grafena diungkap",
    description:
      "Sebuah terobosan baru dalam dunia teknologi telah menggetarkan industri penyimpanan energi. Sebuah tim peneliti di laboratorium teknologi terkemuka di Silicon Valley mengumumkan penemuan terbaru mereka yang bisa mengubah wajah industri baterai.",
  },
];

export const DATA_RECENT_NEWS = [
  {
    imgURL: imgRecent1,
    date: "23 Nov",
    title:
      "Krisis Perubahan Iklim: sinyal terkini yang mendorong tindakan global",
  },
  {
    imgURL: imgRecent2,
    date: "10 Dec",
    title:
      "Revitalisasi Kesehatan Mental: Pergeseran Menuju Kesejahteraan dan Aksesibilitas Holistik",
  },
];

export const DATA_BLOGS = [
  {
    imgURL: imgBlog,
    date: "23 Nov",
    title:
      "Krisis Perubahan Iklim: sinyal terkini yang mendorong tindakan global",
  },
  {
    imgURL: imgBlog,
    date: "10 Dec",
    title:
      "Revitalisasi Kesehatan Mental: Pergeseran Menuju Kesejahteraan dan Aksesibilitas Holistik",
  },
  {
    imgURL: imgBlog,
    date: "23 Nov",
    title:
      "Krisis Perubahan Iklim: sinyal terkini yang mendorong tindakan global",
  },
  {
    imgURL: imgBlog,
    date: "10 Dec",
    title:
      "Revitalisasi Kesehatan Mental: Pergeseran Menuju Kesejahteraan dan Aksesibilitas Holistik",
  },
  {
    imgURL: imgBlog,
    date: "23 Nov",
    title:
      "Krisis Perubahan Iklim: sinyal terkini yang mendorong tindakan global",
  },
  {
    imgURL: imgBlog,
    date: "10 Dec",
    title:
      "Revitalisasi Kesehatan Mental: Pergeseran Menuju Kesejahteraan dan Aksesibilitas Holistik",
  },
];
